...
function filtraDataModeloMuseu({data, chave, pergunta = null}) {
    ...
    switch (true) {
        case chave === 'lista':
            return {listaFilteredSpaces: Object.entries(data).map(([nome, info]) => [nome, info])}

        case chave === 'lista_gratuitos':
            return {listaFilteredSpaces: Object.entries(data).filter(([nome, info]) => info.preco.indexOf("Entrada gratuita") >= 0)}

        case chave === 'lista_pagos':
            return {listaFilteredSpaces: Object.entries(data).filter(([nome, info]) => info.preco.indexOf("Entrada gratuita") < 0)}
            
        case chave === 'lista_horario_funcionamento':
        ...
        case chave === 'lista_horario_funcionamento_agora':
            return {listaFilteredSpaces: listarMuseusAbertos({museus: data, diaAtual: true})};

        case chave === 'lista_horario_funcionamento_dias_de_semana':
            return {listaFilteredSpaces: listarMuseusAbertosDiasUteis(data)};

        case chave === 'lista_horario_funcionamento_fins_de_semana':
            return {listaFilteredSpaces: listarMuseusAbertosFimDeSemana(data)};

        case chave.startsWith("lista_Bairro_"):
            return {listaFilteredSpaces: Object.entries(data).filter(([nome, info]) => 
                removerAcentos(info.endereco).toLowerCase().indexOf(chave.split("lista_Bairro_")[1].toLowerCase()) >= 0)}
        
        case chave.startsWith("extra"):
            palavraChave = chave.split('extra_').join('')
            return {listaFilteredSpaces: Object.entries(data).filter(([nome, info]) => 
                removerAcentos(nome).toLowerCase().split(" ").join('').indexOf(removerAcentos(palavraChave).toLowerCase().split(" ").join('')) >= 0)}
